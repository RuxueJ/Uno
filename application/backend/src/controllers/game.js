...
export async function startGame(roomId, userId) {
  ...
  try {
    ...
    const roomLead = await db.models.roomUser.findOne({
      where: { roomId, userId, isHost: true },
    });
    if (!roomLead) {
...
export async function cleanUpGame(roomId) {
  ...
  try {
    ...
    const playerStates = await db.models.playerState.findAll({
      where: { roomId },
    });
      ...
      if (playerStates.length > 0) {
        await Promise.all(
          playerStates.map((playerState) =>
            playerState.destroy({ transaction })
          )
        );
...
export async function playerDrawCard(roomId, userId) {
  ...
  try {
    ...
    const playerState = await db.models.playerState.findOne({
      where: { roomId, userId },
    });
    if (!playerState) {
    ...
    const currentPlayerIndex = Array.from(playerOrder).findIndex(
    ...
    if (topCard.value === "wild_draw_four") {
      ...
      const index = 0;
      while (
        index < discardDeck.length &&
        discardDeck[index].value === "draw_two"
      ) {
        countNeedToDraw += 2;
        index++;
      }
...
export async function playerPlayCard(roomId, userId, card) {
  ...
  try {
    ...
    const index = playerState.playerHand.findIndex(
      (playerCard) =>
        playerCard.type === card.type &&
        playerCard.color === card.color &&
